...
def _make_plugin(is_ready=True):
    ...
    return plugin, lifecycle, user_state
...
class TestHandleMessage:
    ...
    def test_stop_command_signals_active_stream(self):
        plugin, lifecycle, user_state = _make_plugin()
    ...
    def test_posts_before_ensuring_runtime(self):
        ...
        def ensure_all(_runtime_key, *, model_user_id=None):
            assert plugin.driver.create_post.called
            return "zc-abc.ns.svc.cluster.local"

        lifecycle.ensure_all.side_effect = ensure_all
        with patch("app.bot.plugin.chat_stream", return_value=frames):
            plugin.handle_dm(msg)
    ...
    def test_patches_timeout_on_pod_startup_timeout(self):
        plugin, lifecycle, user_state = _make_plugin(is_ready=False)
        lifecycle.wait_ready.side_effect = TimeoutError
        msg = _make_message()
        plugin.handle_dm(msg)
        last_patch = plugin.driver.posts.patch_post.call_args[0][1]["message"]
    ...
    def test_updates_last_activity_on_success(self):
        ...
        with patch("app.bot.plugin.chat_stream", return_value=frames):
            plugin.handle_dm(msg)
        lifecycle.update_last_activity.assert_called_once()
...
class TestSoulIdentityCommands:
    def test_soul_show_with_override(self):
        plugin, lifecycle, user_state = _make_plugin()
        user_state.get_workspace_file.return_value = "# Custom SOUL"
    ...
    def test_soul_show_without_override(self):
        plugin, lifecycle, user_state = _make_plugin()
        user_state.get_workspace_file.return_value = None
        msg = _make_message(text="!soul show")
        plugin.handle_dm(msg)
        reply = plugin.driver.create_post.call_args[1]["message"]
    ...
    def test_soul_set_posts_edit_button(self):
        plugin, lifecycle, user_state = _make_plugin()
        user_state.get_workspace_file.return_value = ""
    ...
    def test_soul_reset_found(self):
        plugin, lifecycle, user_state = _make_plugin()
        user_state.reset_workspace_file.return_value = True
        msg = _make_message(text="!soul reset")
        plugin.handle_dm(msg)
        user_state.reset_workspace_file.assert_called_once_with("user1", "SOUL.md")
        reply = plugin.driver.create_post.call_args[1]["message"]
        assert "✅" in reply

    def test_soul_reset_not_found(self):
        plugin, lifecycle, user_state = _make_plugin()
        user_state.reset_workspace_file.return_value = False
        msg = _make_message(text="!soul reset")
        plugin.handle_dm(msg)
        reply = plugin.driver.create_post.call_args[1]["message"]
        assert "✅" not in reply
    ...
    def test_identity_reset_calls_correct_filename(self):
        ...
        plugin.handle_dm(msg)
        user_state.reset_workspace_file.assert_called_once_with("user1", "IDENTITY.md")
...
class TestModelCommands:
    def test_model_show_returns_current_model(self):
        plugin, lifecycle, user_state = _make_plugin()
        user_state.get_user_model.return_value = "gpt-4o"
        msg = _make_message(text="!model show")

        plugin.handle_dm(msg)

        lifecycle.ensure_all.assert_not_called()
        user_state.get_user_model.assert_called_once_with("user1")
        reply = plugin.driver.create_post.call_args[1]["message"]
    ...
    def test_model_set_accepts_allowed_model(self):
        plugin, lifecycle, user_state = _make_plugin()
        user_state.set_user_model.return_value = True
        msg = _make_message(text="!model set gpt-4o")

        plugin.handle_dm(msg)

        user_state.set_user_model.assert_called_once_with("user1", "gpt-4o")
        reply = plugin.driver.create_post.call_args[1]["message"]
        assert "gpt-4o" in reply
        assert "✅" in reply
    ...
    def test_model_set_rejects_unknown_model(self):
        plugin, lifecycle, user_state = _make_plugin()
        user_state.set_user_model.return_value = False
        msg = _make_message(text="!model set bad-model")

        plugin.handle_dm(msg)

        user_state.set_user_model.assert_called_once_with("user1", "bad-model")
        reply = plugin.driver.create_post.call_args[1]["message"]
    ...
    def test_model_reset_calls_runtime(self):
        plugin, lifecycle, user_state = _make_plugin()
        user_state.reset_user_model.return_value = True
        msg = _make_message(text="!model reset")

        plugin.handle_dm(msg)

        user_state.reset_user_model.assert_called_once_with("user1")
        reply = plugin.driver.create_post.call_args[1]["message"]
        assert "✅" in reply
...
class TestApprovalRequests:
    def test_returns_always_decision(self):
        ...
        thread = threading.Thread(
            target=lambda: result.setdefault(
                "decision",
                plugin._approval.request(
                    frame, channel_id="ch1", root_id="root1", main_post_id="post-123"
                ),
            )
        )
        thread.start()
        deadline = time.monotonic() + 1
        while "req-1" not in plugin._approval._pending and time.monotonic() < deadline:
            time.sleep(0.01)
    ...
    def test_returns_timeout_decision(self):
        ...
        decision = plugin._approval.request(
            frame, channel_id="ch1", root_id="root1", main_post_id="post-123"
        )

        assert decision == "timeout"
        assert "req-2" not in plugin._approval._pending
...
class TestChannelHandling:
    ...
    def test_channel_handler_uses_channel_id_as_runtime_key(self):
        ...
        with patch("app.bot.plugin.chat_stream", return_value=frames):
            plugin.handle_channel_mention(msg)
        lifecycle.ensure_all.assert_called_once_with("channel-abc", model_user_id="user1")
    ...
    def test_channel_handler_updates_activity_with_channel_id(self):
        ...
        with patch("app.bot.plugin.chat_stream", return_value=frames):
            plugin.handle_channel_mention(msg)
        lifecycle.update_last_activity.assert_called_with("channel-abc")
